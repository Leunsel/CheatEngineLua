--[[
    ...
    Menus want a TImageList and an index per item. The record tree and the
    results strip draw their own glyphs on a canvas at whatever y a row sits
    at, and a list index is no use there. Each icon is composited once onto an
    opaque bitmap of the row's background colour and cached, by icon then by
    background, and the cache is dropped when the palette moves, which is the
    only time a row background changes. The LCL will alpha blend a
    TPortableNetworkGraphic through Canvas.Draw on Win32, but that is a
    StretchMaskBlt per glyph, per row, per repaint, and a build that does not
    blend draws a black tile.

    The artwork is copied byte identical from the sibling segments. Nothing
    here generates art. Every segment owns its own folder so it stays a
    directory that can be copied.

    API facts, out of the Cheat Engine 7.5 and Lazarus sources.
      * createPNG(w,h) returns a TPortableNetworkGraphic, descending
        TFPImageBitmap, TCustomBitmap, TGraphic. Valid for both
        customimagelist_add, which casts to TCustomBitmap, and canvas.draw,
        which wants a TGraphic.
      * createBitmap().loadFromFile('x.png') does not work. TBitmap reads BMP
        only and TGraphic.LoadFromFile does no format sniffing, so the BMP
        reader runs over PNG bytes and raises.
      * createPicture() is a TPicture, not a TGraphic. Passing one to add() or
        to canvas.draw is an unchecked pointer type pun. Unused here.
      * graphic.loadFromFile has no try except in Cheat Engine's binding, so it
        is pcall'd and the PNG magic is checked before the decoder sees a file.
      ...
      * A size mismatch stretches rather than centring or clipping. The PNGs
        and the list are both 16x16.

    None of this is required for the window to work. Every entry point reports
    failure and the caller falls back to no icon at all.
]]
...
Icons.ModulesFolder = "Manifold-AddressList-Modules"
Icons.IconSize = 16

--- Logical name to file. The names are this segment's own verbs, the files are
--- the shared Manifold artwork. A name with no file, or a file that will not
--- load, degrades to no icon.
Icons.Files = {
    -- Toolbar verbs
    Refresh     = "Manifold-FullReload.png",
    Live        = "Manifold-Follow.png",
    Undo        = "Manifold-Reset.png",
    Redo        = "Manifold-Reload.png",
    Find        = "Manifold-Search.png",
    Problems    = "Manifold-Validate.png",
    ...
    Clear       = "Manifold-Clear-Filters.png",
    Eye         = "Manifold-Eye.png",
    Detail      = "Manifold-Detail.png",
    Script      = "Manifold-Development.png",
    Hotkey      = "Manifold-Level.png",
    Changes     = "Manifold-Recent.png",
...
--- @param options table|nil # Root overrides the autorun path, for tests.
--- @return table
--
function Icons:New(options)
    if not instance then
        instance = setmetatable({
            Root = nil,
            List = nil,
            Index = {},      -- name to image list index
            Graphics = {},   -- name to TPortableNetworkGraphic, for the canvas
            Composites = {}, -- name to background to opaque TBitmap
            CompositeCount = 0,
            Loaded = false,
            Reason = nil,
            Missing = {}
...
--
--- ∑ True when the file exists and really begins with the PNG signature. The
---   Cheat Engine binding of graphic.loadFromFile has no try except, so the
---   decoder must never be handed something it cannot read.
--- @param path string
...
---   failure is remembered rather than retried on every menu build.
--- @return boolean, string|nil
--
function Icons:Load()
    ...
    -- Height go through the published property RTTI fallback, so read them
...
---   Lazarus version, and lua_setProperty stashes unknown property writes in
---   the userdata's metatable inside a try except, so writing item.ImageList
...
---   owning menu's Images. An item's own SubMenuImages never applies to
---   itself, only to its children.
...
--
--- ∑ Sets an icon on one item. TMenuItem.SetImageIndex early exits when the
---   new value equals the old, and again when no image list resolves. So write
---   -1 first to force a real transition, and only after SubMenuImages is
...
function Icons:Apply(item, name)
    ...
    -- Menu glyphs can be suppressed by Application.ShowMenuGlyphs or by the
    -- operating system's show menu icons setting. Ask for them explicitly. A
    -- failure is ignored, the enum is only reachable through the RTTI
    -- fallback.
...
---   runs once per icon and background rather than once per row per repaint.
--- @param name string
...
--
--- ∑ Draws one icon at x and y on a canvas.
